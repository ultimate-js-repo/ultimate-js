export function serialize(value: unknown): unknown {
  ...
  if (value instanceof ReadableStream) {
    throw new Error("Cannot serialize: ReadableStream");
  }
...
export function deserialize(value: unknown): unknown {
  ...
  if (obj.__type === "Map") {
    const entries = (obj.entries as Array<[unknown, unknown]>).map(([k, v]) =>
      [deserialize(k), deserialize(v)] as [unknown, unknown]
    );
