...
- **Errors** — `UltimateError`, `BadRequestError`, `NotFoundError`,
...
- **Diagnostics** — `Diagnostic`, `formatDiagnostic()`, `addDiagnostic()`,
  `hasErrors()`
- **Config** — `defineConfig()`, `resolveConfig()`, `loadConfig()`,
  `UltimateConfig`, `ResolvedConfig`
