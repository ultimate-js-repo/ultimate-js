...
import { createSuccessResult, createFailureResult, validateRemoteCall, serializeError } from "@ultimate-js/protocol";
...
export function createRpcHandler(options: {
  ...
  return async (request: Request, explicitFunctionId?: string): Promise<Response> => {
    if (request.method === "OPTIONS") {
      return withCors(null, { status: 204 });
    }

    if (request.method !== "POST") {
      return withCors(JSON.stringify(createFailureResult({
    ...
    try {
      ...
      return withCors(JSON.stringify(failure), { status: 400, headers: { "Content-Type": "application/json" } });
    }
    ...
    if (!fn) {
      ...
      return withCors(JSON.stringify(failure), { status: 404, headers: { "Content-Type": "application/json" } });
    }
    ...
    try {
      ...
      return withCors(JSON.stringify(result), { status: 200, headers: { "Content-Type": "application/json" } });
    } catch (err) {
      const failure = createFailureResult(serializeError(err, dev));
      const statusCode = (err instanceof Error && "statusCode" in err && typeof (err as Error & { statusCode: number }).statusCode === "number")
        ? (err as Error & { statusCode: number }).statusCode
        : 500;
      return withCors(JSON.stringify(failure), { status: statusCode, headers: { "Content-Type": "application/json" } });
    }
  };
}
