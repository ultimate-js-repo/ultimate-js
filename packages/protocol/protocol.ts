...
export function createRemoteCall(args: unknown[], meta?: RemoteFunctionCalling["meta"]): RemoteFunctionCalling {
...
export function createFailureResult(error: { name: string; message: string; statusCode?: number; code?: string; stack?: string }): RemoteFunctionResult {
...
export function validateRemoteCall(body: unknown): RemoteFunctionCalling {
  ...
  if (b.type !== "RemoteFunctionCalling") throw new Error(`Expected type RemoteFunctionCalling, got ${b.type}`);
